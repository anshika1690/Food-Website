const restaurants = [

  {
    "image": "second",
    "name": "Ocean's Catch",
    "rating": 3,
    "foodType": "Chinese",
    "price_for_two": 2279,
    "location": "Hauz Khas",
    "distance_from_customer_house": "4.8",
    "offer": 40,
    "alcohol": false,
    "restaurant_open_time": 11,
    "restaurant_close_time": 23
  },
  {
    "image": "five",
    "name": "The Gourmet Spot",
    "rating": 4,
    "foodType": "Thai",
    "price_for_two": 1690,
    "location": "Saket",
    "distance_from_customer_house": "1.5",
    "offer": 10,
    "alcohol": false,
    "restaurant_open_time": 3,
    "restaurant_close_time": 15
  },
  {
    "image": "ten",
    "name": "Green Leaf Café",
    "rating": 2,
    "foodType": "French",
    "price_for_two": 1567,
    "location": "Chandni Chowk",
    "distance_from_customer_house": "1.2",
    "offer": 30,
    "alcohol": false,
    "restaurant_open_time": 21,
    "restaurant_close_time": 9
  },
  {
    "image": "second",
    "name": "Sweet Cravings",
    "rating": 3,
    "foodType": "Mexican",
    "price_for_two": 2340,
    "location": "Karol Bagh",
    "distance_from_customer_house": "8.2",
    "offer": 20,
    "alcohol": false,
    "restaurant_open_time": 14,
    "restaurant_close_time": 2
  },
  {
    "image": "seven",
    "name": "Ocean's Catch",
    "rating": 4,
    "foodType": "Indian",
    "price_for_two": 908,
    "location": "Dwarka",
    "distance_from_customer_house": "10.4",
    "offer": 30,
    "alcohol": true,
    "restaurant_open_time": 12,
    "restaurant_close_time": 0
  },
  {
    "image": "eight",
    "name": "Spice Symphony",
    "rating": 5,
    "foodType": "Indian",
    "price_for_two": 1220,
    "location": "Saket",
    "distance_from_customer_house": "9.4",
    "offer": 25,
    "alcohol": false,
    "restaurant_open_time": 4,
    "restaurant_close_time": 16
  },
  {
    "image": "second",
    "name": "Royal Curry House",
    "rating": 3,
    "foodType": "Italian",
    "price_for_two": 2180,
    "location": "Vasant Kunj",
    "distance_from_customer_house": "10.2",
    "offer": 30,
    "alcohol": false,
    "restaurant_open_time": 4,
    "restaurant_close_time": 16
  },
  {
    "image": "ten",
    "name": "Golden Spoon",
    "rating": 5,
    "foodType": "Japanese",
    "price_for_two": 1382,
    "location": "Saket",
    "distance_from_customer_house": "5.6",
    "offer": 10,
    "alcohol": true,
    "restaurant_open_time": 17,
    "restaurant_close_time": 5
  },
  {
    "image": "first",
    "name": "Bistro Delights",
    "rating": 5,
    "foodType": "Greek",
    "price_for_two": 579,
    "location": "South Extension",
    "distance_from_customer_house": "6.3",
    "offer": 40,
    "alcohol": false,
    "restaurant_open_time": 0,
    "restaurant_close_time": 12
  },
  {
    "image": "second",
    "name": "Fusion Feast",
    "rating": 4,
    "foodType": "Indian",
    "price_for_two": 1584,
    "location": "Connaught Place",
    "distance_from_customer_house": "5.6",
    "offer": 25,
    "alcohol": false,
    "restaurant_open_time": 5,
    "restaurant_close_time": 17
  },
  {
    "image": "eight",
    "name": "Flavors of Italy",
    "rating": 3,
    "foodType": "Chinese",
    "price_for_two": 1935,
    "location": "Chandni Chowk",
    "distance_from_customer_house": "5.8",
    "offer": 30,
    "alcohol": true,
    "restaurant_open_time": 11,
    "restaurant_close_time": 23
  },
  {
    "image": "third",
    "name": "Taste Haven",
    "rating": 1,
    "foodType": "Greek",
    "price_for_two": 1448,
    "location": "Chandni Chowk",
    "distance_from_customer_house": "4.1",
    "offer": 25,
    "alcohol": true,
    "restaurant_open_time": 10,
    "restaurant_close_time": 22
  },
  {
    "image": "second",
    "name": "Fire & Smoke BBQ",
    "rating": 1,
    "foodType": "Chinese",
    "price_for_two": 1504,
    "location": "Dwarka",
    "distance_from_customer_house": "4.7",
    "offer": 25,
    "alcohol": false,
    "restaurant_open_time": 9,
    "restaurant_close_time": 21
  },
  {
    "image": "ten",
    "name": "Ocean's Catch",
    "rating": 2,
    "foodType": "Thai",
    "price_for_two": 1810,
    "location": "Chandni Chowk",
    "distance_from_customer_house": "1.7",
    "offer": 40,
    "alcohol": true,
    "restaurant_open_time": 22,
    "restaurant_close_time": 10
  },
  {
    "image": "seven",
    "name": "The Gourmet Spot",
    "rating": 4,
    "foodType": "Mediterranean",
    "price_for_two": 121,
    "location": "Chandni Chowk",
    "distance_from_customer_house": "10.1",
    "offer": 30,
    "alcohol": false,
    "restaurant_open_time": 9,
    "restaurant_close_time": 21
  },
  {
    "image": "four",
    "name": "Fire & Smoke BBQ",
    "rating": 2,
    "foodType": "Mediterranean",
    "price_for_two": 1780,
    "location": "Vasant Kunj",
    "distance_from_customer_house": "7.5",
    "offer": 25,
    "alcohol": false,
    "restaurant_open_time": 6,
    "restaurant_close_time": 18
  },
  {
    "image": "five",
    "name": "The Cozy Nook",
    "rating": 5,
    "foodType": "Japanese",
    "price_for_two": 542,
    "location": "Rajouri Garden",
    "distance_from_customer_house": "9.1",
    "offer": 10,
    "alcohol": false,
    "restaurant_open_time": 15,
    "restaurant_close_time": 3
  },
  {
    "image": "second",
    "name": "The Rustic Table",
    "rating": 3,
    "foodType": "Indian",
    "price_for_two": 1007,
    "location": "Dwarka",
    "distance_from_customer_house": "6.7",
    "offer": 10,
    "alcohol": false,
    "restaurant_open_time": 14,
    "restaurant_close_time": 2
  },
  {
    "image": "second",
    "name": "Sushi Zen",
    "rating": 2,
    "foodType": "Greek",
    "price_for_two": 1305,
    "location": "Chandni Chowk",
    "distance_from_customer_house": "9.7",
    "offer": 25,
    "alcohol": false,
    "restaurant_open_time": 23,
    "restaurant_close_time": 11
  },
  {
    "image": "five",
    "name": "Green Leaf Café",
    "rating": 3,
    "foodType": "Greek",
    "price_for_two": 735,
    "location": "Saket",
    "distance_from_customer_house": "7.8",
    "offer": 30,
    "alcohol": false,
    "restaurant_open_time": 9,
    "restaurant_close_time": 21
  },
  {
    "image": "seven",
    "name": "The Hungry Fork",
    "rating": 5,
    "foodType": "Indian",
    "price_for_two": 2413,
    "location": "South Extension",
    "distance_from_customer_house": "8.9",
    "offer": 30,
    "alcohol": false,
    "restaurant_open_time": 11,
    "restaurant_close_time": 23
  },
  {
    "image": "third",
    "name": "Sweet Cravings",
    "rating": 4,
    "foodType": "American",
    "price_for_two": 328,
    "location": "Lajpat Nagar",
    "distance_from_customer_house": "2.0",
    "offer": 20,
    "alcohol": false,
    "restaurant_open_time": 14,
    "restaurant_close_time": 2
  },
  {
    "image": "third",
    "name": "Sushi Zen",
    "rating": 1,
    "foodType": "Mediterranean",
    "price_for_two": 717,
    "location": "Hauz Khas",
    "distance_from_customer_house": "8.0",
    "offer": 40,
    "alcohol": false,
    "restaurant_open_time": 9,
    "restaurant_close_time": 21
  },
  {
    "image": "six",
    "name": "Sweet Cravings",
    "rating": 2,
    "foodType": "Thai",
    "price_for_two": 859,
    "location": "Karol Bagh",
    "distance_from_customer_house": "4.5",
    "offer": 10,
    "alcohol": true,
    "restaurant_open_time": 5,
    "restaurant_close_time": 17
  },
  {
    "image": "ten",
    "name": "Fusion Feast",
    "rating": 4,
    "foodType": "French",
    "price_for_two": 484,
    "location": "Karol Bagh",
    "distance_from_customer_house": "1.8",
    "offer": 40,
    "alcohol": false,
    "restaurant_open_time": 8,
    "restaurant_close_time": 20
  },
  {
    "image": "ten",
    "name": "Fire & Smoke BBQ",
    "rating": 2,
    "foodType": "Mexican",
    "price_for_two": 212,
    "location": "Lajpat Nagar",
    "distance_from_customer_house": "8.6",
    "offer": 10,
    "alcohol": false,
    "restaurant_open_time": 17,
    "restaurant_close_time": 5
  },
  {
    "image": "second",
    "name": "The Gourmet Spot",
    "rating": 3,
    "foodType": "Indian",
    "price_for_two": 1564,
    "location": "Chandni Chowk",
    "distance_from_customer_house": "9.6",
    "offer": 25,
    "alcohol": false,
    "restaurant_open_time": 4,
    "restaurant_close_time": 16
  },
  {
    "image": "seven",
    "name": "Fire & Smoke BBQ",
    "rating": 1,
    "foodType": "Indian",
    "price_for_two": 1666,
    "location": "Vasant Kunj",
    "distance_from_customer_house": "1.5",
    "offer": 50,
    "alcohol": false,
    "restaurant_open_time": 16,
    "restaurant_close_time": 4
  },
  {
    "image": "four",
    "name": "Sunset Diner",
    "rating": 4,
    "foodType": "Greek",
    "price_for_two": 340,
    "location": "Connaught Place",
    "distance_from_customer_house": "5.2",
    "offer": 50,
    "alcohol": false,
    "restaurant_open_time": 18,
    "restaurant_close_time": 6
  },
  {
    "image": "six",
    "name": "Bistro Delights",
    "rating": 5,
    "foodType": "Mediterranean",
    "price_for_two": 1104,
    "location": "Hauz Khas",
    "distance_from_customer_house": "1.2",
    "offer": 50,
    "alcohol": false,
    "restaurant_open_time": 3,
    "restaurant_close_time": 15
  },
  {
    "image": "third",
    "name": "The Rustic Table",
    "rating": 3,
    "foodType": "American",
    "price_for_two": 302,
    "location": "Lajpat Nagar",
    "distance_from_customer_house": "2.0",
    "offer": 30,
    "alcohol": false,
    "restaurant_open_time": 13,
    "restaurant_close_time": 1
  },
  {
    "image": "eight",
    "name": "Flavors of Italy",
    "rating": 2,
    "foodType": "American",
    "price_for_two": 1465,
    "location": "Karol Bagh",
    "distance_from_customer_house": "5.9",
    "offer": 20,
    "alcohol": false,
    "restaurant_open_time": 15,
    "restaurant_close_time": 3
  },
  {
    "image": "six",
    "name": "Sweet Cravings",
    "rating": 5,
    "foodType": "Japanese",
    "price_for_two": 1856,
    "location": "Rajouri Garden",
    "distance_from_customer_house": "4.7",
    "offer": 30,
    "alcohol": false,
    "restaurant_open_time": 20,
    "restaurant_close_time": 8
  },
  {
    "image": "nine",
    "name": "The Cozy Nook",
    "rating": 3,
    "foodType": "Italian",
    "price_for_two": 2376,
    "location": "Connaught Place",
    "distance_from_customer_house": "7.0",
    "offer": 40,
    "alcohol": false,
    "restaurant_open_time": 21,
    "restaurant_close_time": 9
  },
  {
    "image": "third",
    "name": "Sunset Diner",
    "rating": 2,
    "foodType": "Mediterranean",
    "price_for_two": 843,
    "location": "Hauz Khas",
    "distance_from_customer_house": "4.1",
    "offer": 20,
    "alcohol": false,
    "restaurant_open_time": 8,
    "restaurant_close_time": 20
  },
  {
    "image": "first",
    "name": "Royal Curry House",
    "rating": 4,
    "foodType": "American",
    "price_for_two": 1008,
    "location": "Rajouri Garden",
    "distance_from_customer_house": "1.9",
    "offer": 25,
    "alcohol": false,
    "restaurant_open_time": 4,
    "restaurant_close_time": 16
  },
  {
    "image": "six",
    "name": "Fire & Smoke BBQ",
    "rating": 2,
    "foodType": "Mediterranean",
    "price_for_two": 736,
    "location": "Lajpat Nagar",
    "distance_from_customer_house": "10.2",
    "offer": 10,
    "alcohol": true,
    "restaurant_open_time": 8,
    "restaurant_close_time": 20
  },
  {
    "image": "five",
    "name": "Fusion Feast",
    "rating": 4,
    "foodType": "French",
    "price_for_two": 544,
    "location": "Karol Bagh",
    "distance_from_customer_house": "4.9",
    "offer": 25,
    "alcohol": false,
    "restaurant_open_time": 4,
    "restaurant_close_time": 16
  },
  {
    "image": "seven",
    "name": "Taste Haven",
    "rating": 5,
    "foodType": "Japanese",
    "price_for_two": 1288,
    "location": "Lajpat Nagar",
    "distance_from_customer_house": "5.7",
    "offer": 25,
    "alcohol": false,
    "restaurant_open_time": 8,
    "restaurant_close_time": 20
  },
  {
    "image": "seven",
    "name": "Ocean's Catch",
    "rating": 1,
    "foodType": "French",
    "price_for_two": 1184,
    "location": "South Extension",
    "distance_from_customer_house": "11.0",
    "offer": 25,
    "alcohol": false,
    "restaurant_open_time": 5,
    "restaurant_close_time": 17
  },
  {
    "image": "nine",
    "name": "The Cozy Nook",
    "rating": 4,
    "foodType": "Mediterranean",
    "price_for_two": 2374,
    "location": "South Extension",
    "distance_from_customer_house": "6.8",
    "offer": 20,
    "alcohol": false,
    "restaurant_open_time": 9,
    "restaurant_close_time": 21
  },
  {
    "image": "second",
    "name": "Green Leaf Café",
    "rating": 5,
    "foodType": "Chinese",
    "price_for_two": 1008,
    "location": "Connaught Place",
    "distance_from_customer_house": "1.7",
    "offer": 40,
    "alcohol": false,
    "restaurant_open_time": 10,
    "restaurant_close_time": 22
  },
  {
    "image": "four",
    "name": "Fire & Smoke BBQ",
    "rating": 2,
    "foodType": "Japanese",
    "price_for_two": 730,
    "location": "Hauz Khas",
    "distance_from_customer_house": "1.8",
    "offer": 25,
    "alcohol": false,
    "restaurant_open_time": 1,
    "restaurant_close_time": 13
  },
  {
    "image": "second",
    "name": "The Cozy Nook",
    "rating": 1,
    "foodType": "French",
    "price_for_two": 2086,
    "location": "Vasant Kunj",
    "distance_from_customer_house": "2.0",
    "offer": 30,
    "alcohol": true,
    "restaurant_open_time": 16,
    "restaurant_close_time": 4
  },
  {
    "image": "six",
    "name": "Sizzling Wok",
    "rating": 1,
    "foodType": "Mediterranean",
    "price_for_two": 768,
    "location": "Chandni Chowk",
    "distance_from_customer_house": "4.2",
    "offer": 30,
    "alcohol": false,
    "restaurant_open_time": 17,
    "restaurant_close_time": 5
  },
  {
    "image": "five",
    "name": "The Cozy Nook",
    "rating": 4,
    "foodType": "Indian",
    "price_for_two": 2010,
    "location": "Lajpat Nagar",
    "distance_from_customer_house": "10.5",
    "offer": 50,
    "alcohol": true,
    "restaurant_open_time": 8,
    "restaurant_close_time": 20
  },
  {
    "image": "four",
    "name": "Taste Haven",
    "rating": 2,
    "foodType": "Thai",
    "price_for_two": 564,
    "location": "Karol Bagh",
    "distance_from_customer_house": "9.3",
    "offer": 20,
    "alcohol": false,
    "restaurant_open_time": 23,
    "restaurant_close_time": 11
  },
  {
    "image": "five",
    "name": "Sunset Diner",
    "rating": 4,
    "foodType": "American",
    "price_for_two": 788,
    "location": "Vasant Kunj",
    "distance_from_customer_house": "10.9",
    "offer": 10,
    "alcohol": false,
    "restaurant_open_time": 17,
    "restaurant_close_time": 5
  },
  {
    "image": "four",
    "name": "The Cozy Nook",
    "rating": 5,
    "foodType": "French",
    "price_for_two": 1035,
    "location": "Saket",
    "distance_from_customer_house": "9.6",
    "offer": 10,
    "alcohol": false,
    "restaurant_open_time": 23,
    "restaurant_close_time": 11
  },
  {
    "image": "six",
    "name": "Steakhouse Supreme",
    "rating": 1,
    "foodType": "Chinese",
    "price_for_two": 351,
    "location": "Chandni Chowk",
    "distance_from_customer_house": "7.5",
    "offer": 25,
    "alcohol": true,
    "restaurant_open_time": 9,
    "restaurant_close_time": 21
  },
  {
    "image": "second",
    "name": "Fusion Feast",
    "rating": 5,
    "foodType": "American",
    "price_for_two": 1048,
    "location": "Lajpat Nagar",
    "distance_from_customer_house": "10.1",
    "offer": 30,
    "alcohol": false,
    "restaurant_open_time": 12,
    "restaurant_close_time": 0
  },
  {
    "image": "eight",
    "name": "Urban Bites",
    "rating": 2,
    "foodType": "American",
    "price_for_two": 898,
    "location": "Dwarka",
    "distance_from_customer_house": "5.2",
    "offer": 25,
    "alcohol": false,
    "restaurant_open_time": 12,
    "restaurant_close_time": 0
  },
  {
    "image": "five",
    "name": "Sweet Cravings",
    "rating": 2,
    "foodType": "Mexican",
    "price_for_two": 1751,
    "location": "Vasant Kunj",
    "distance_from_customer_house": "4.8",
    "offer": 20,
    "alcohol": true,
    "restaurant_open_time": 7,
    "restaurant_close_time": 19
  },
  {
    "image": "second",
    "name": "Sunset Diner",
    "rating": 3,
    "foodType": "Indian",
    "price_for_two": 972,
    "location": "Lajpat Nagar",
    "distance_from_customer_house": "7.5",
    "offer": 25,
    "alcohol": false,
    "restaurant_open_time": 21,
    "restaurant_close_time": 9
  },
  {
    "image": "ten",
    "name": "Royal Curry House",
    "rating": 2,
    "foodType": "Greek",
    "price_for_two": 1156,
    "location": "South Extension",
    "distance_from_customer_house": "9.6",
    "offer": 25,
    "alcohol": false,
    "restaurant_open_time": 6,
    "restaurant_close_time": 18
  },
  {
    "image": "six",
    "name": "Royal Curry House",
    "rating": 1,
    "foodType": "Thai",
    "price_for_two": 106,
    "location": "Chandni Chowk",
    "distance_from_customer_house": "6.0",
    "offer": 10,
    "alcohol": false,
    "restaurant_open_time": 5,
    "restaurant_close_time": 17
  },
  {
    "image": "first",
    "name": "Ocean's Catch",
    "rating": 2,
    "foodType": "Greek",
    "price_for_two": 681,
    "location": "Dwarka",
    "distance_from_customer_house": "7.7",
    "offer": 50,
    "alcohol": true,
    "restaurant_open_time": 4,
    "restaurant_close_time": 16
  },
  {
    "image": "ten",
    "name": "Sizzling Wok",
    "rating": 3,
    "foodType": "Japanese",
    "price_for_two": 1095,
    "location": "Saket",
    "distance_from_customer_house": "5.4",
    "offer": 30,
    "alcohol": true,
    "restaurant_open_time": 20,
    "restaurant_close_time": 8
  },
  {
    "image": "four",
    "name": "Golden Spoon",
    "rating": 4,
    "foodType": "Mediterranean",
    "price_for_two": 1102,
    "location": "Chandni Chowk",
    "distance_from_customer_house": "8.0",
    "offer": 25,
    "alcohol": true,
    "restaurant_open_time": 0,
    "restaurant_close_time": 12
  },
  {
    "image": "first",
    "name": "Bistro Delights",
    "rating": 4,
    "foodType": "American",
    "price_for_two": 2046,
    "location": "Connaught Place",
    "distance_from_customer_house": "6.3",
    "offer": 40,
    "alcohol": false,
    "restaurant_open_time": 13,
    "restaurant_close_time": 1
  },
  {
    "image": "third",
    "name": "Fusion Feast",
    "rating": 4,
    "foodType": "Greek",
    "price_for_two": 1261,
    "location": "Lajpat Nagar",
    "distance_from_customer_house": "6.3",
    "offer": 30,
    "alcohol": true,
    "restaurant_open_time": 4,
    "restaurant_close_time": 16
  },
  {
    "image": "four",
    "name": "The Cozy Nook",
    "rating": 5,
    "foodType": "Thai",
    "price_for_two": 621,
    "location": "South Extension",
    "distance_from_customer_house": "4.9",
    "offer": 10,
    "alcohol": false,
    "restaurant_open_time": 12,
    "restaurant_close_time": 0
  },
  {
    "image": "nine",
    "name": "The Hungry Fork",
    "rating": 1,
    "foodType": "Mexican",
    "price_for_two": 1114,
    "location": "Connaught Place",
    "distance_from_customer_house": "2.5",
    "offer": 20,
    "alcohol": true,
    "restaurant_open_time": 13,
    "restaurant_close_time": 1
  },
  {
    "image": "four",
    "name": "Green Leaf Café",
    "rating": 4,
    "foodType": "Chinese",
    "price_for_two": 579,
    "location": "Karol Bagh",
    "distance_from_customer_house": "8.7",
    "offer": 30,
    "alcohol": false,
    "restaurant_open_time": 1,
    "restaurant_close_time": 13
  },
  {
    "image": "four",
    "name": "The Rustic Table",
    "rating": 5,
    "foodType": "American",
    "price_for_two": 891,
    "location": "Saket",
    "distance_from_customer_house": "1.5",
    "offer": 10,
    "alcohol": true,
    "restaurant_open_time": 6,
    "restaurant_close_time": 18
  },
  {
    "image": "second",
    "name": "Fusion Feast",
    "rating": 1,
    "foodType": "French",
    "price_for_two": 517,
    "location": "Saket",
    "distance_from_customer_house": "8.7",
    "offer": 30,
    "alcohol": false,
    "restaurant_open_time": 17,
    "restaurant_close_time": 5
  },
  {
    "image": "nine",
    "name": "The Cozy Nook",
    "rating": 1,
    "foodType": "Mediterranean",
    "price_for_two": 1999,
    "location": "Lajpat Nagar",
    "distance_from_customer_house": "5.1",
    "offer": 40,
    "alcohol": false,
    "restaurant_open_time": 8,
    "restaurant_close_time": 20
  },
  {
    "image": "four",
    "name": "Sunset Diner",
    "rating": 1,
    "foodType": "Indian",
    "price_for_two": 340,
    "location": "Rajouri Garden",
    "distance_from_customer_house": "8.6",
    "offer": 30,
    "alcohol": false,
    "restaurant_open_time": 18,
    "restaurant_close_time": 6
  },
  {
    "image": "ten",
    "name": "Sizzling Wok",
    "rating": 1,
    "foodType": "Indian",
    "price_for_two": 227,
    "location": "Hauz Khas",
    "distance_from_customer_house": "2.7",
    "offer": 10,
    "alcohol": true,
    "restaurant_open_time": 12,
    "restaurant_close_time": 0
  },
  {
    "image": "nine",
    "name": "Ocean's Catch",
    "rating": 3,
    "foodType": "Chinese",
    "price_for_two": 936,
    "location": "Connaught Place",
    "distance_from_customer_house": "4.0",
    "offer": 30,
    "alcohol": false,
    "restaurant_open_time": 20,
    "restaurant_close_time": 8
  },
  {
    "image": "first",
    "name": "Taste Haven",
    "rating": 2,
    "foodType": "Italian",
    "price_for_two": 1518,
    "location": "South Extension",
    "distance_from_customer_house": "3.6",
    "offer": 40,
    "alcohol": false,
    "restaurant_open_time": 17,
    "restaurant_close_time": 5
  },
  {
    "image": "four",
    "name": "The Hungry Fork",
    "rating": 5,
    "foodType": "French",
    "price_for_two": 1024,
    "location": "Dwarka",
    "distance_from_customer_house": "1.6",
    "offer": 10,
    "alcohol": false,
    "restaurant_open_time": 2,
    "restaurant_close_time": 14
  },
  {
    "image": "third",
    "name": "The Rustic Table",
    "rating": 3,
    "foodType": "Italian",
    "price_for_two": 1215,
    "location": "Rajouri Garden",
    "distance_from_customer_house": "9.8",
    "offer": 50,
    "alcohol": true,
    "restaurant_open_time": 12,
    "restaurant_close_time": 0
  },
  {
    "image": "third",
    "name": "Fire & Smoke BBQ",
    "rating": 4,
    "foodType": "Thai",
    "price_for_two": 523,
    "location": "Vasant Kunj",
    "distance_from_customer_house": "6.7",
    "offer": 30,
    "alcohol": false,
    "restaurant_open_time": 2,
    "restaurant_close_time": 14
  },
  {
    "image": "ten",
    "name": "Golden Spoon",
    "rating": 4,
    "foodType": "Mediterranean",
    "price_for_two": 1760,
    "location": "South Extension",
    "distance_from_customer_house": "6.6",
    "offer": 10,
    "alcohol": false,
    "restaurant_open_time": 4,
    "restaurant_close_time": 16
  },
  {
    "image": "second",
    "name": "Royal Curry House",
    "rating": 4,
    "foodType": "French",
    "price_for_two": 526,
    "location": "Dwarka",
    "distance_from_customer_house": "10.0",
    "offer": 20,
    "alcohol": false,
    "restaurant_open_time": 4,
    "restaurant_close_time": 16
  },
  {
    "image": "nine",
    "name": "Sunset Diner",
    "rating": 1,
    "foodType": "Chinese",
    "price_for_two": 1108,
    "location": "Lajpat Nagar",
    "distance_from_customer_house": "1.5",
    "offer": 50,
    "alcohol": false,
    "restaurant_open_time": 13,
    "restaurant_close_time": 1
  },
  {
    "image": "second",
    "name": "The Hungry Fork",
    "rating": 3,
    "foodType": "Mexican",
    "price_for_two": 1442,
    "location": "Chandni Chowk",
    "distance_from_customer_house": "1.1",
    "offer": 20,
    "alcohol": false,
    "restaurant_open_time": 6,
    "restaurant_close_time": 18
  },
  {
    "image": "eight",
    "name": "Spice Symphony",
    "rating": 3,
    "foodType": "American",
    "price_for_two": 183,
    "location": "Saket",
    "distance_from_customer_house": "2.6",
    "offer": 30,
    "alcohol": false,
    "restaurant_open_time": 3,
    "restaurant_close_time": 15
  },
  {
    "image": "eight",
    "name": "Steakhouse Supreme",
    "rating": 1,
    "foodType": "French",
    "price_for_two": 1167,
    "location": "Dwarka",
    "distance_from_customer_house": "6.1",
    "offer": 30,
    "alcohol": false,
    "restaurant_open_time": 13,
    "restaurant_close_time": 1
  },
  {
    "image": "eight",
    "name": "The Cozy Nook",
    "rating": 2,
    "foodType": "American",
    "price_for_two": 1409,
    "location": "Vasant Kunj",
    "distance_from_customer_house": "10.9",
    "offer": 50,
    "alcohol": true,
    "restaurant_open_time": 15,
    "restaurant_close_time": 3
  },
  {
    "image": "ten",
    "name": "The Hungry Fork",
    "rating": 5,
    "foodType": "American",
    "price_for_two": 2376,
    "location": "Connaught Place",
    "distance_from_customer_house": "8.6",
    "offer": 50,
    "alcohol": true,
    "restaurant_open_time": 5,
    "restaurant_close_time": 17
  },
  {
    "image": "second",
    "name": "Taste Haven",
    "rating": 5,
    "foodType": "Indian",
    "price_for_two": 1128,
    "location": "Karol Bagh",
    "distance_from_customer_house": "8.8",
    "offer": 50,
    "alcohol": true,
    "restaurant_open_time": 13,
    "restaurant_close_time": 1
  },
  {
    "image": "nine",
    "name": "The Gourmet Spot",
    "rating": 5,
    "foodType": "Indian",
    "price_for_two": 1561,
    "location": "Hauz Khas",
    "distance_from_customer_house": "4.5",
    "offer": 10,
    "alcohol": false,
    "restaurant_open_time": 4,
    "restaurant_close_time": 16
  },
  {
    "image": "four",
    "name": "The Cozy Nook",
    "rating": 5,
    "foodType": "Mediterranean",
    "price_for_two": 602,
    "location": "Hauz Khas",
    "distance_from_customer_house": "10.2",
    "offer": 20,
    "alcohol": false,
    "restaurant_open_time": 13,
    "restaurant_close_time": 1
  },
  {
    "image": "eight",
    "name": "Sushi Zen",
    "rating": 4,
    "foodType": "Indian",
    "price_for_two": 1561,
    "location": "Chandni Chowk",
    "distance_from_customer_house": "4.0",
    "offer": 10,
    "alcohol": false,
    "restaurant_open_time": 7,
    "restaurant_close_time": 19
  },
  {
    "image": "seven",
    "name": "Sunset Diner",
    "rating": 5,
    "foodType": "Indian",
    "price_for_two": 2192,
    "location": "South Extension",
    "distance_from_customer_house": "6.9",
    "offer": 50,
    "alcohol": false,
    "restaurant_open_time": 2,
    "restaurant_close_time": 14
  },
  {
    "image": "second",
    "name": "Sizzling Wok",
    "rating": 5,
    "foodType": "Italian",
    "price_for_two": 929,
    "location": "Chandni Chowk",
    "distance_from_customer_house": "8.1",
    "offer": 50,
    "alcohol": false,
    "restaurant_open_time": 7,
    "restaurant_close_time": 19
  },
  {
    "image": "four",
    "name": "Golden Spoon",
    "rating": 2,
    "foodType": "Italian",
    "price_for_two": 1069,
    "location": "Rajouri Garden",
    "distance_from_customer_house": "4.1",
    "offer": 10,
    "alcohol": false,
    "restaurant_open_time": 23,
    "restaurant_close_time": 11
  },
  {
    "image": "first",
    "name": "Spice Symphony",
    "rating": 4,
    "foodType": "Greek",
    "price_for_two": 1962,
    "location": "South Extension",
    "distance_from_customer_house": "2.4",
    "offer": 30,
    "alcohol": false,
    "restaurant_open_time": 19,
    "restaurant_close_time": 7
  },
  {
    "image": "seven",
    "name": "Fusion Feast",
    "rating": 4,
    "foodType": "Japanese",
    "price_for_two": 1190,
    "location": "South Extension",
    "distance_from_customer_house": "6.0",
    "offer": 10,
    "alcohol": false,
    "restaurant_open_time": 0,
    "restaurant_close_time": 12
  },
  {
    "image": "nine",
    "name": "Steakhouse Supreme",
    "rating": 3,
    "foodType": "Japanese",
    "price_for_two": 1036,
    "location": "South Extension",
    "distance_from_customer_house": "4.8",
    "offer": 10,
    "alcohol": true,
    "restaurant_open_time": 4,
    "restaurant_close_time": 16
  },
  {
    "image": "seven",
    "name": "Sweet Cravings",
    "rating": 3,
    "foodType": "Indian",
    "price_for_two": 1281,
    "location": "Karol Bagh",
    "distance_from_customer_house": "7.8",
    "offer": 30,
    "alcohol": true,
    "restaurant_open_time": 16,
    "restaurant_close_time": 4
  },
  {
    "image": "five",
    "name": "Sweet Cravings",
    "rating": 1,
    "foodType": "Japanese",
    "price_for_two": 2335,
    "location": "South Extension",
    "distance_from_customer_house": "2.1",
    "offer": 40,
    "alcohol": true,
    "restaurant_open_time": 10,
    "restaurant_close_time": 22
  },
  {
    "image": "four",
    "name": "Bistro Delights",
    "rating": 2,
    "foodType": "Italian",
    "price_for_two": 1179,
    "location": "Karol Bagh",
    "distance_from_customer_house": "7.0",
    "offer": 25,
    "alcohol": false,
    "restaurant_open_time": 19,
    "restaurant_close_time": 7
  },
  {
    "image": "five",
    "name": "Spice Symphony",
    "rating": 5,
    "foodType": "French",
    "price_for_two": 1728,
    "location": "Saket",
    "distance_from_customer_house": "11.0",
    "offer": 25,
    "alcohol": false,
    "restaurant_open_time": 3,
    "restaurant_close_time": 15
  },
  {
    "image": "third",
    "name": "Sweet Cravings",
    "rating": 4,
    "foodType": "French",
    "price_for_two": 645,
    "location": "Hauz Khas",
    "distance_from_customer_house": "5.1",
    "offer": 10,
    "alcohol": false,
    "restaurant_open_time": 13,
    "restaurant_close_time": 1
  },
  {
    "image": "ten",
    "name": "Taste Haven",
    "rating": 5,
    "foodType": "French",
    "price_for_two": 1029,
    "location": "Vasant Kunj",
    "distance_from_customer_house": "9.0",
    "offer": 30,
    "alcohol": true,
    "restaurant_open_time": 2,
    "restaurant_close_time": 14
  },
  {
    "image": "four",
    "name": "The Rustic Table",
    "rating": 5,
    "foodType": "Mediterranean",
    "price_for_two": 980,
    "location": "Karol Bagh",
    "distance_from_customer_house": "10.1",
    "offer": 40,
    "alcohol": true,
    "restaurant_open_time": 6,
    "restaurant_close_time": 18
  },
  {
    "image": "ten",
    "name": "Fusion Feast",
    "rating": 3,
    "foodType": "Greek",
    "price_for_two": 1057,
    "location": "Rajouri Garden",
    "distance_from_customer_house": "3.7",
    "offer": 10,
    "alcohol": false,
    "restaurant_open_time": 23,
    "restaurant_close_time": 11
  }
]

function getRestaurant(restaurants) {

  const root = document.getElementById('root');


  //iterating over every restaurant

  restaurants.forEach((restaurant) => {
    //create a card
    //1. image
    //2. card_content div
    //three div in card_content
    // i. card_header (Name and rating)
    // ii. card_footer (foodType and price)
    // iii. card_location (restaurant location, user distance)


    //create card
    const card = document.createElement('div');
    card.classList.add('card');

    //create image
    const image = document.createElement('img');
    image.src = `images/${restaurant.image}.avif`;

    //create offer
    const offer = document.createElement('span');
    offer.classList.add('offer');
    if (restaurant.offer == 20) {
      offer.textContent = "Flat 20% OFF";
      card.appendChild(offer);
    }
    else if (restaurant.offer == 50) {
      offer.textContent = "Flat 50% OFF";
      card.appendChild(offer);
    }
    else if (restaurant.offer == 30) {
      offer.textContent = "Flat 30% OFF";
      card.appendChild(offer);
    }
    
    

    //create card_content div
    const Card_content = document.createElement('div');
    Card_content.classList.add('card-content');


    //making sub div
    //card header
    const Card_header = document.createElement('div');
    Card_header.classList.add('card-header');

    const h3 = document.createElement('h3');
    h3.textContent = restaurant.name;

    //span tag
    const rate = document.createElement('span');
    rate.classList.add('rating');
    rate.textContent = "Rating: " + restaurant.rating;


    Card_header.appendChild(h3);
    Card_header.appendChild(rate);



    //card footer
    const Card_footer = document.createElement('div');
    Card_footer.classList.add('card-footer');

    const food = document.createElement('span');
    food.textContent = restaurant.foodType;

    const price = document.createElement('span');
    price.textContent = "₹" + restaurant.price_for_two;

    //appending
    Card_footer.appendChild(food);
    Card_footer.appendChild(price);


    //card location
    const Card_location = document.createElement('div');
    Card_location.classList.add('card-location');

    const location = document.createElement('span');
    location.textContent = restaurant.location;

    const distance = document.createElement('span');
    distance.textContent = restaurant.distance_from_customer_house + " km";

    //appending
    Card_location.appendChild(location);
    Card_location.appendChild(distance);


    //appending to card content
    Card_content.appendChild(Card_header);
    Card_content.appendChild(Card_footer);
    Card_content.appendChild(Card_location);


    //appending to card
    card.appendChild(image);
    card.appendChild(Card_content);

    //appending to root
    root.appendChild(card);
  })

}
getRestaurant(restaurants);


//showing alcohol restaurant
document.getElementById('alcohol').addEventListener('click', () => {
  const result = restaurants.filter((obj) => obj.alcohol);  //for true value go to result

  // getRestaurant(result);

  //when click on serve alcohol existing data will remain in webpage and filtered ones will go in down
  //that why previous ones has to be removed

  // document.getElementById('root').innerHTML = "";   //below line same work of this
  document.getElementById('root').replaceChildren();
  getRestaurant(result);


})



//rating filter
document.getElementById('rating').addEventListener('click', () => {
  const result = restaurants.filter((obj) => obj.rating > 4.5);

  document.getElementById('root').replaceChildren();
  getRestaurant(result);
})



//open now
document.getElementById('open').addEventListener('click', () => {
  const result = restaurants.filter((obj) => obj.restaurant_open_time >= 8 && obj.restaurant_open_time <= 22);

  document.getElementById('root').replaceChildren();
  getRestaurant(result);
})


//offer
document.getElementById('offers').addEventListener('click', () => {
  const result = restaurants.filter((obj) => obj.offer == 30 || obj.offer == 20 || obj.offer == 50);

  document.getElementById('root').replaceChildren();
  getRestaurant(result);
  
})



//filters button
document.getElementById('filters').addEventListener('click', () => {
  document.getElementById('filterPopup').classList.remove('hidden');
})


//listen on apply button
document.getElementById('applyFilter').addEventListener('click', () => {
  
  // jb apply pr click karungi then jo bhi radio button select hoga vo uthna hai

  const element = document.querySelector('input[name="filterOption"]:checked');
  const answer = element.value;

  if (answer == "rating") {
    // restaurants.sort((a, b) => a.rating - b.rating);    //ascending
    restaurants.sort((a, b) => b.rating - a.rating);    //decending

  }
  else if (answer == "highLow") {
    restaurants.sort((a, b) => b.price_for_two - a.price_for_two);
  }
  else if (answer == "lowHigh") {
    restaurants.sort((a, b) => a.price_for_two - b.price_for_two);
  }
  else if (answer == "distance") {   //distance kam hona chahiye
    restaurants.sort((a, b) => a.distance_from_customer_house - b.distance_from_customer_house);
  }

  document.getElementById('root').replaceChildren();
  document.getElementById('filterPopup').classList.add('hidden');
  getRestaurant(restaurants);
  
})

document.getElementById('closeFilter').addEventListener('click', () => {
  document.getElementById('filterPopup').classList.add('hidden');
  
})


